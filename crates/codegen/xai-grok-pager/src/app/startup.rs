...
///
/// Returns whether a process-global prefetch is in flight (started or already
/// registered), matching [`startup_prefetch::begin`].
pub fn kick_auth_and_prefetch(grok_com_config: GrokComConfig) -> bool {
    ...
    xai_grok_shell::agent::models::startup_prefetch::begin(Some(grok_com_config))
}

/// Join the process-global prefetch for remote settings (capped wait).
pub fn join_early_prefetch(had_prefetch: bool) -> Option<RemoteSettings> {
    if !had_prefetch {
        return None;
    }
