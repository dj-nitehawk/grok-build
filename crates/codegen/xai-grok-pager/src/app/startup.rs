...
//! A cache hit or a still-valid token applies remote settings without blocking
//! first paint. Refresh is not awaited first, so an expired token and a cold
//! settings cache leave remote settings unset for this process.
//!
//! First paint shows a real welcome shell (non-minimal) so startup feels instant.
//! Keys typed on that frozen frame are startup type-ahead, replayed when the
//! event loop starts. The frame itself does not read input.
//!
...
///
/// Returns the effective root plus the layers that produced it, so AppInit can
/// pin features without a second merge.
pub fn reload_config_after_remote(
    existing: toml::Value,
    existing_layers: xai_grok_shell::config::EffectiveConfigLayers,
    remote: Option<&RemoteSettings>,
) -> anyhow::Result<(toml::Value, xai_grok_shell::config::EffectiveConfigLayers)> {
    if remote.is_some() {
        xai_grok_shell::config::load_effective_config_with_layers()
            .map(|(layers, value)| (value, layers))
            .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))
    } else {
        Ok((existing, existing_layers))
    }
}
...
///
/// Keys typed while this frame is up are drained later into startup type-ahead.
/// This paint does not read the terminal queue.
