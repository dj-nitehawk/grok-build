...
fn dispatch_handoff_while_in_progress_refuses_second_request() {
    ...
    assert!(
        effects.is_empty(),
        "second handoff must not stack: {effects:?}"
    );
    let agent = &app.agents[&AgentId(0)];
    let toast = agent.toast.as_ref().expect("refusal toast");
    assert!(toast.0.contains("already in progress"), "got: {}", toast.0);
...
fn handoff_ready_creates_empty_peer_and_seeds_prompt() {
    ...
    // Simulate the loading entry that dispatch_handoff would have created.
    {
        ...
        let id = agent
            ...
            .push(crate::scrollback::entry::ScrollbackEntry::running(
                RenderBlock::system("Generating handoff note \u{2014} continue shipping"),
            ));
    ...
    assert!(
        child
            ...
            .pending_prompts
            .iter()
            .any(|p| { p.text.contains("Ship handoff") && p.text.contains("continue shipping") }),
...
fn handoff_failed_toasts_on_parent() {
    let mut app = fork_test_app();
    {
        ...
        let id = agent
            .scrollback
            .push(crate::scrollback::entry::ScrollbackEntry::running(
                RenderBlock::system("Generating handoff note \u{2014} task"),
            ));
        agent.pending_handoff_entry = Some(id);
    }
    let effects = dispatch(
