...
pub async fn run(
    ...
    let raw_config = startup::reload_config_after_remote(raw_config, remote_settings.as_ref())?;
    ...
    if !args.leader
        ...
        && remote_settings.as_ref().and_then(|s| s.leader_mode) == Some(false)
    {
        tokio::spawn(xai_grok_shell::leader::kill_stale_reachable_leaders(
            "remote",
        ));
    }
