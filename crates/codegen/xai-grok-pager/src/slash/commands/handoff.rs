...
impl SlashCommand for HandoffCommand {
    slash_meta! {
        name: "handoff",
        description: "Hand off task-relevant context to a new session",
        usage: "/handoff <task for the new session>",
        takes_args: true,
        args_required: true,
        session_scoped: true,
        arg_placeholder: "<task>",
    }

    fn run(&self, _ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        let task = args.trim();
        if task.is_empty() {
            return CommandResult::Error("Usage: /handoff <task for the new session>".into());
        }
