...
pub fn format_quota_chip_at(balance: &CreditBalance, now: chrono::DateTime<chrono::Utc>) -> String {
    let pct = balance.usage_pct.floor() as i64;
    match balance.period_end {
        Some(end) if end <= now => format!("{pct}% (reset due; refresh quota)"),
        Some(end) => {
...
mod tests {
    ...
    fn format_quota_chip_with_and_without_period_end() {
        ...
        assert_eq!(format_quota_chip_at(&with_end, now), "10% (reset: 4d5h)");
        assert_eq!(format_quota_chip_at(&bal(42.9), now), "42%");
        for period_end in [now, now - chrono::Duration::seconds(1)] {
            let expired = CreditBalance {
                period_end: Some(period_end),
                ..bal(10.4)
            };
            assert_eq!(
                format_quota_chip_at(&expired, now),
                "10% (reset due; refresh quota)"
            );
        }
    }
    ...
    fn prompt_border_chips_order_context_mode_quota() {
        ...
        assert!(
            woven[0].text.contains('/'),
            "context first: {}",
            woven[0].text
        );
        assert_eq!(woven[1].text, "plan");
        assert!(
            woven[2].text.starts_with("10%"),
            "quota last: {}",
            woven[2].text
        );
        assert!(
