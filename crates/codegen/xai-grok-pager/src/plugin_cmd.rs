...
const MARKETPLACE_DISABLED: &str =
    "marketplace support is not compiled into this build (rebuild with --features marketplace)";
...
enum PluginEntry {
    ...
    #[cfg_attr(not(feature = "marketplace"), allow(dead_code))]
    Available {
        name: String,
        version: Option<String>,
...
pub async fn run(args: PluginArgs) -> Result<()> {
    match args.command {
        ...
        PluginCommand::Marketplace(mp) => {
            #[cfg(feature = "marketplace")]
            {
                run_marketplace(mp.command).await
            }
            #[cfg(not(feature = "marketplace"))]
            {
                let _ = mp;
                bail!("{MARKETPLACE_DISABLED}")
            }
        }
    }
}
...
#[cfg(not(feature = "marketplace"))]
fn available_plugins(_registry: &InstallRegistry) -> Vec<PluginEntry> {
    Vec::new()
}
...
#[cfg(feature = "marketplace")]
#[path = "plugin_cmd_marketplace.rs"]
mod plugin_cmd_marketplace;
#[cfg(feature = "marketplace")]
use plugin_cmd_marketplace::*;

#[cfg(all(test, feature = "marketplace"))]
mod tests {
    use super::plugin_cmd_marketplace::{find_removal_source, marketplace_update_with_cache_root};
    use super::*;
