...
## Use GPT-6 Astra or GPT-5.6 Sol

```bash
grok models          # lists gpt-6-astra and gpt-5.6-sol plus the usual Grok defaults
grok -m gpt-5.6-sol -p "reply with pong"
```

In a session: `/model gpt-5.6-sol` (or `gpt-6-astra`) or Ctrl+M. After picking a Codex model, `/model` chains into a reasoning-effort menu. Default is `medium`. Codex rejects `none` (HTTP 400). Sol also offers `max`. You can set effort later with `/effort high`. Switching Grok ↔ GPT compact-on-family-change (`xai` → `openai`).
...
Seeded slugs are `gpt-6-astra` and `gpt-5.6-sol`. There are no extra gpt-5.x aliases (`gpt-5.6` is not seeded). If the Codex backend rejects the slug, that is a live eligibility failure, not a missing fallback.

## Override

User `[model.gpt-6-astra]` or `[model."gpt-5.6-sol"]` still wins via the normal merge. Quote the Sol key; the `.` in `gpt-5.6-sol` is a TOML table separator if left bare. Example:
