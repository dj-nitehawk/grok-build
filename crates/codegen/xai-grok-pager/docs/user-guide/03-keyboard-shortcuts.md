...
| `Alt+Q` | Agent screen | Refresh subscription quota for the active Grok or ChatGPT/Codex model. Grok shows `10% (reset: 4d5h)`; ChatGPT shows available windows such as `5h: 10% (reset: 2h)` and `7d: 25% (reset: 4d)`. Shows `refreshing...` while fetching. |

Quota successes are cached for 60 seconds independently per provider. Repeated presses share an in-flight request; failures can be retried immediately and retain the previous snapshot. ChatGPT marks a retained snapshot as stale when refreshing fails; expired reset times ask you to refresh quota rather than continuing a countdown. Switching models preserves warm caches but never shows another provider's quota. There is no automatic polling. ChatGPT uses the identity from `grok chatgpt-login`, independently of Grok subscription eligibility; changing accounts or logging out invalidates its cached display. Missing windows are omitted. A single usable ChatGPT window shows `25% (reset: 4d)` without a duration label, like Grok; multiple windows retain their duration labels. Missing usable data shows `ChatGPT quota unavailable`. Other providers and OpenAI API-key billing are unsupported. `/usage` remains the Grok usage modal and shares that 60-second cache and in-flight request. ChatGPT quota uses an unofficial backend endpoint and may become unavailable if that contract changes.
