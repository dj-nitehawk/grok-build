...
| `Alt+Q` | Agent screen | Refresh Grok usage quota on the prompt bottom border (`10% (reset: 4d5h)`). Shows `refreshing...` while the fetch is in flight. Cached for 1 minute — further presses and `/usage` share that cache and one in-flight fetch. |
