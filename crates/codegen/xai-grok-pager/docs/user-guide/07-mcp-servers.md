...
- The qualified name must also be a valid model function name: start with a letter or underscore, and be at most 64 characters. Longer catalog keys are not promoted. They stay on `search_tool` / `use_tool`, and grok logs a warning. Sending an over-long name as a native tool makes the provider reject the whole request.
