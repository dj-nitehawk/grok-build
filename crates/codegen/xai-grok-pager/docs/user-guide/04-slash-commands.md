...
Prints how to sign in with ChatGPT Plus/Pro for `gpt-6-astra`. The OAuth callback binds `localhost:1455`, so the flow runs in a terminal (`grok chatgpt-login`, or `--device` if that port is busy). See [ChatGPT Plus/Pro](chatgpt.md).
