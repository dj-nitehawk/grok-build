//! Non-exporting OpenTelemetry layer when feature `export-otel` is off.
//!
//! Keeps type names and function paths stable for shell/pager/bin constructors.
//! Spans still mint W3C ids for sampling and ACP. Nothing is exported.
...
/// Builds a non-exporting tracer layer (OTLP export compiled out).
...
{
    xai_grok_otel::non_exporting_tracer_layer()
}
