...
pub(crate) const SOL_MODEL_ID: &str = "gpt-6-sol";
pub(crate) const SOL_MODEL_DISPLAY_NAME: &str = "GPT-6 Sol";
...
pub(crate) fn stamp_sampler_config_from_url(config: &mut SamplerConfig) {
    stamp_reconstructed_sampler_config(config, None);
}

/// Reattach ChatGPT credentials after chat-state reconstruction.
///
/// Resolvers and header injectors are not stored on the sampling config, so a
/// later turn has to stamp them again. Official Codex URLs are detected from
/// `base_url`. An explicit `auth_provider = "chatgpt"` proxy is not, and must
/// be passed here or the live turn drops the bearer (and will not enforce HTTPS).
pub(crate) fn stamp_reconstructed_sampler_config(
    config: &mut SamplerConfig,
    auth_provider: Option<&str>,
) {
    extras::apply_extras_policy(config);
    if extras::is_codex_backend_url(&config.base_url) || auth_provider == Some(AUTH_PROVIDER_NAME) {
        attach_credentials(config);
    }
}
...
fn serialize_token(auth: &store::ChatgptAuth) -> anyhow::Result<String> {
    ...
    if let Some(expires_in) = auth.expires_in_secs() {
        output
            ...
            .insert("expires_in".into(), serde_json::json!(expires_in));
    }
...
mod tests {
    ...
    fn catalog_survives_remote_prefetch_replace() {
        ...
        assert!(
            ...
            "prefetch must not drop gpt-6-sol"
        );
...
"#,
    ...
    #[test]
    fn reconstructed_stamp_reattaches_chatgpt_provider_on_a_non_codex_url() {
        let mut config = SamplerConfig {
            base_url: "https://proxy.example/v1".into(),
            api_key: Some("cached-token".into()),
            include_encrypted_reasoning: true,
            responses_system_as_instructions: false,
            ..Default::default()
        };
        // Chat-state rebuilds do not persist the resolver. URL-only stamping
        // leaves an explicit chatgpt proxy unauthenticated.
        stamp_sampler_config_from_url(&mut config);
        assert!(config.bearer_resolver.is_none());
        assert_eq!(config.api_key.as_deref(), Some("cached-token"));

        stamp_reconstructed_sampler_config(&mut config, Some("not-chatgpt"));
        assert!(config.bearer_resolver.is_none());

        stamp_reconstructed_sampler_config(&mut config, Some(AUTH_PROVIDER_NAME));
        assert_eq!(
            config
                .bearer_resolver
                .as_ref()
        ...
        assert!(config.header_injector.is_some());
        assert_eq!(config.api_key.as_deref(), Some("cached-token"));
        assert!(config.include_encrypted_reasoning);
        assert!(!config.responses_system_as_instructions);

        config.base_url = "http://proxy.example/v1".into();
        config.api_key = Some("must-not-leak".into());
        stamp_reconstructed_sampler_config(&mut config, Some(AUTH_PROVIDER_NAME));
        assert!(config.api_key.is_none());
        let resolver = config.bearer_resolver.as_ref().unwrap();
        assert!(resolver.current_bearer().is_none());
        assert!(resolver.auth_provider_name().is_none());
    }

    #[test]
    fn wire_id_collision_does_not_donate_the_synthetic_chatgpt_provider() {
        let entries = default_entries();
        let astra = entries.get(MODEL_ID).unwrap();
        assert_eq!(
            crate::agent::config::matching_route_auth_provider(astra, &astra.info.base_url)
                .as_deref(),
            Some(AUTH_PROVIDER_NAME)
        );
        assert!(
            crate::agent::config::matching_route_auth_provider(astra, "https://proxy.example/v1")
                .is_none(),
            "a user route that only reuses the wire id must keep its own credentials"
        );
        let grok = entries
            .values()
            .find(|entry| entry.info.model.starts_with("grok"))
            .expect("grok default");
        let collided = crate::agent::config::byok_for_live_route_decision(
            crate::agent::auth_method::ModelByok::NotByok,
            &grok.info.base_url,
            "https://proxy.example/v1",
        );
        assert_eq!(collided, crate::agent::auth_method::ModelByok::Unknown);
        assert_eq!(
            crate::agent::config::byok_for_live_route_decision(
                crate::agent::auth_method::ModelByok::NotByok,
                &grok.info.base_url,
                &grok.info.base_url,
            ),
