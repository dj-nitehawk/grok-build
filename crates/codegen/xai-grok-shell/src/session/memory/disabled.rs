...
pub mod storage {
    ...
    pub use xai_grok_config_types::MemoryMode;
    ...
    impl MemoryStorage {
        ...
        pub fn new_for_mode(cwd: &Path, root_override: Option<&Path>, _mode: MemoryMode) -> Self {
            Self::new(cwd, root_override)
        }
        ...
        pub fn save_remember_note(&self, _content: &str) -> Result<(), std::io::Error> {
            Err(std::io::Error::other(not_compiled()))
        }
...
pub mod dream_lock {
    ...
    impl DreamLock {
        ...
        pub fn try_acquire(&self, _stale_secs: u64) -> std::io::Result<Option<String>> {
            Ok(Some(String::new()))
        }
        pub fn acquire(&self, _stale_secs: u64) -> std::io::Result<Option<DreamLockGuard>> {
            Ok(Some(DreamLockGuard))
        }
...
pub mod dream {
    ...
    pub fn execute_dream(
        ...
        DreamResult {
            status: DreamStatus::Failed(not_compiled()),
            sessions_eligible,
        }
    }

    pub fn clean_processed_sessions(_sessions_dir: &Path, _stems: &[String]) -> Vec<String> {
        Vec::new()
    }
}
...
pub mod flush {
    ...
    pub fn process_flush_response(_response: &str, _config: &MemoryFlushConfig) -> FlushResult {
...
impl V2MemoryAccessPolicy {
    pub fn new(_global_root: &Path, _workspace_root: &Path) -> Result<Self, String> {
        Ok(Self)
    }
}

impl xai_grok_tools::types::memory_v2::MemoryV2Access for V2MemoryAccessPolicy {
    fn validate_read(&self, _path: &Path) -> Result<bool, String> {
        Ok(false)
    }
    fn record_read(&self, _path: &Path, _contents: &[u8]) -> Result<(), String> {
        Ok(())
    }
    fn preflight_write(&self, _path: &Path, _contents: &[u8]) -> Result<bool, String> {
        Ok(false)
    }
    fn write_file(
        &self,
        _path: &Path,
        _contents: &[u8],
    ) -> Result<xai_grok_tools::types::memory_v2::MemoryV2Write, String> {
...
pub fn regenerate_scope_manifest(
    _scope_dir: &Path,
    _scope: V2MemoryScope,
    _budget: V2ManifestBudget,
