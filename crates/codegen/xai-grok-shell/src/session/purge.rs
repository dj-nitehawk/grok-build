...
///
/// `dir` must be a real directory. A symlink is refused so `/purge` cannot
/// sweep whatever `sessions/` or `logs/` points at. Symlink entries are
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn clear_directory_contents_refuses_symlink_dir() {
        let tmp = tempfile::tempdir().unwrap();
        let real = tmp.path().join("real-sessions");
        fs::create_dir(&real).unwrap();
        fs::write(real.join("keep.txt"), b"secret").unwrap();
        let link = tmp.path().join("sessions");
        std::os::unix::fs::symlink(&real, &link).unwrap();
        let err = clear_directory_contents(&link).unwrap_err();
        assert!(err.to_string().contains("symlink"), "{err}");
        assert_eq!(fs::read(real.join("keep.txt")).unwrap(), b"secret");
    }

    #[cfg(unix)]
    #[test]
    fn clear_directory_contents_unlinks_symlink_entries_without_following() {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().join("sessions");
        fs::create_dir(&root).unwrap();
        let outside = tmp.path().join("outside");
        fs::create_dir(&outside).unwrap();
        fs::write(outside.join("keep.txt"), b"secret").unwrap();
        std::os::unix::fs::symlink(&outside, root.join("linked-dir")).unwrap();
        std::os::unix::fs::symlink(outside.join("keep.txt"), root.join("linked-file")).unwrap();
        let n = clear_directory_contents(&root).unwrap();
        assert_eq!(n, 2);
        assert_eq!(fs::read(outside.join("keep.txt")).unwrap(), b"secret");
