...
- **ChatGPT Codex:** do not send a Grok session bearer to chatgpt.com. Codex entries must `has_own_credentials()` (synthetic `auth_provider.chatgpt`). First-party Responses extras (`reasoning.encrypted_content`, hosted search includes) are gated in `agent/chatgpt/extras.rs`; Codex 400s on unknown `include` values. Codex also 400s on `role: system` in `input` ("System messages are not allowed"); extras stamps `responses_system_as_instructions` and `apply_response_defaults` lifts those items into the top-level `instructions` field (Codex CLI's shape). Do not drop the Grok system prompt. OAuth client id `app_EMoamEEZ73f0CkXaXp7hrann` is unofficial; do not spoof a Codex User-Agent without an explicit decision. Callback port 1455 clashes with OpenCode/Codex CLI (`--device` fallback). Remote prefetch *replaces* the default map; `chatgpt::merge_catalog` must run after that assign or `gpt-6-astra` / `gpt-5.6-sol` vanish from `/model`. Astra reasoning: `xhigh`/`high`/`medium` (default)/`low`. Sol also advertises `max`. Do not offer `none` (Codex HTTP 400). Those lists are what `/model` shows after picking a Codex model. User overrides for Sol must quote the key (`[model."gpt-5.6-sol"]`); a bare `[model.gpt-5.6-sol]` is nested TOML, not the catalog id.
