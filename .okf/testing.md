...
| --- | --- | --- |
| `xai-grok-shell` | empty | `workflows`, `memory`, `pdf`, `lsp`, `cloud-upload`, `hub-telemetry`, `image-extra`, `web-fetch`, `pptx`, `system-power`, `prometheus-metrics`, `codebase-graph`, `foreign-sessions` |
| `xai-grok-pager` | mainly `sandbox-enforce` | `mermaid`, `voice`, `image-extra`, `foreign-sessions` |
| `xai-grok-tools` | minimal (`serde` etc.) | `pdf`, `lsp`, `pptx`, `web-fetch`, `workflows`, `memory`, `image-extra` |
| `xai-grok-pager-bin` | slim (`sandbox-enforce`); use `product-full` for fat | jemalloc, mermaid, voice, shell leaf forwards, telemetry export, crash-report, foreign-sessions |
...
| `foreign-sessions` | pager `app/foreign_sessions`, `dispatch/session/foreign`, `import_claude_modal`; shell `claude_import` |
