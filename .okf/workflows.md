...
cargo check -p <crate>
# Slim composition root (`grok-slim-check`: no default features except sandbox-enforce) or fat opt-in
...
cargo grok-slim   # release-dist + --no-default-features --features sandbox-enforce (fork alias)
```
...
- **Release body:** the H1 in `crates/codegen/xai-grok-shell/CHANGELOG.md` whose version equals the tag with the leading `v` removed (current H1 is `# 1.0.38`). The job fails if that section is missing. Auto PR/commit notes are off. Ignore the old `v0.2.x` sample in the workflow comments.
...
git checkout dev
git pull
git tag v1.0.38   # lowercase v; version must match a changelog H1
