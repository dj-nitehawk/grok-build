...
| ChatGPT login | `grok chatgpt-login` / `--device` | Writes `~/.grok/chatgpt-auth.json`; port 1455 |
...
Sandbox profiles (`off`, `workspace`, `read-only`, `strict`, `devbox`, …) change FS/network rights for the agent process; default is off. User `/settings` writes to `config.toml` still persist under write-deny profiles via a privileged helper; agent tools/bash cannot.
...
| `~/.grok/chatgpt-auth.json` | ChatGPT Codex OAuth tokens (separate from `auth.json`) |
...
Runtime setting precedence, highest first (user guide `05-configuration.md`):

1. CLI flags
2. Direct environment variables (`XAI_API_KEY`, `GROK_MEMORY`, ...)
3. `requirements.toml` / MDM (clamps every layer below, including the overlay)
4. `GROK_CONFIG` / `GROK_CONFIG_PATH` overlay (deep-merge, allowlisted soft settings; not a permission path)
5. `config.toml`
6. `managed_config.toml`
7. Built-in defaults

File-tier merge, lowest to highest: `managed_config.toml`, then `config.toml`, then the overlay, then requirements/MDM clamp. `GROK_CONFIG` wins over `GROK_CONFIG_PATH` when both are set.

Notable env var **names** (not values): `GROK_HOME`, `XAI_API_KEY`, `GROK_MEMORY`, `GROK_CONFIG`, `GROK_CONFIG_PATH`, `PROTOC`.
...
Auth: browser-based login on first launch for interactive use (`grok login`). ChatGPT Plus/Pro for `gpt-6-astra` is `grok chatgpt-login` (unofficial Codex backend). Security reports via HackerOne only.
