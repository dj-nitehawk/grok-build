...
| Path | Contents |
| --- | --- |
| `crates/codegen/` | Primary product crates: pager, shell, tools, workspace, config, MCP, and the other workspace members |
...
| ChatGPT Codex backend | `crates/codegen/xai-grok-shell/src/agent/chatgpt/` (catalog, OAuth store, extras, sampler stamp). CLI: `xai-grok-pager-bin/src/chatgpt_login.rs`. Slash: `slash/commands/chatgpt_login.rs` |
| Provider-aware Alt+Q | Shell `agent/chatgpt/quota.rs`, `extensions/chatgpt_quota.rs`; pager `app/provider_quota.rs`, `app/effects/chatgpt_quota.rs`, `views/credit_bar.rs`. Shared routers contain delegation only. |
