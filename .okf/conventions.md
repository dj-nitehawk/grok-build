...
- Config layering: highest priority first is CLI, direct env, `requirements.toml` / MDM, `GROK_CONFIG` overlay, `config.toml`, `managed_config.toml`, defaults. `requirements.toml` clamps the layers below it. File merge lives in `xai-grok-config`. See [Operations](operations.md#config-and-observability).
...
6. **Always-keep files stay fork-owned.** Never “refresh” `templates/prompt.md` / `subagent_prompt.md` / root `README.md` from `main` to shrink the diff. See [Fork Sync always-keep](fork-sync.md#always-keep-fork-files).
...
8. **OKF stays in `setup okf`.** Do not edit `.okf/**` inside feature area commits (release CI, slim, TTFP, …). Document the feature in OKF and fold those paths into `setup okf` so later folds/rebases do not fight over the same markdown.
9. **No plugin framework just for isolation.** Prefer the patterns above over inventing a generalization layer unless product needs it.
...
- [Fork Sync](fork-sync.md) (branch model, always-keep files, sync checklist)
