...
| Toolchain | `rust-toolchain.toml` channel `1.94.0` (+ rustfmt, clippy). Release workflow `toolchain` input must match this channel |
...
| Git | fork pin `gix` **0.87** (main is `0.83`; drop the pin when main is `>= 0.86`). `xai-gix-status`, `xai-fast-worktree` (`gix-status` **0.34**). Sunset: [Fork Sync](fork-sync.md#sunset-gix-086-gix-odb-2723) |
