...
- Per-crate `Cargo.toml` `[features]` and `optional = true` deps. `xai-grok-mcp`: take main's `rmcp` version; keep `reqwest-tls-no-provider` and reqwest `rustls-no-provider` (not main's `reqwest` / `rustls`)
