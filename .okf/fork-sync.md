...
18. [ ] **gix sunset:** `git show main:Cargo.toml | rg '^gix = '`. If `>= 0.86`, drop series area 13 and this checklist item (fold the OKF delete into `setup okf`). If `< 0.86`, keep the bump commit at `0.87` (not `0.86`: yanked `bisync`; expect `Cargo.lock` conflicts)
