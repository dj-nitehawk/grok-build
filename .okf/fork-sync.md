...
One commit per series area (currently ~14). Fold within an area:
...
cargo check -p xai-grok-pager-bin --no-default-features --message-format=short
...
| Syntax highlight | workspace syntect `default-fancy` | **on (lighter)** | keep highlighting; drop Oniguruma (`onig`/`onig_sys`); pure-Rust fancy-regex. Main may switch the workspace pin to `default-onig`; keep `default-fancy` |
...
- Markdown `syntax.rs`: take main Swift patched grammar (`patched_swift`, `OUT_DIR/syntaxes.bin`) and keep fork `resolve_syntax_token` csharp/fsharp aliases plus their tests
...
Temporary fork fix. Under workspace/read-only/strict, kernel write-deny of `~/.grok/config.toml` (H1-3969489) blocked TUI `/settings` and `/model` saves. The area forks a privileged writer before bwrap/Seatbelt so user-initiated persist works; agent path writes stay denied.

**Area commit:** `fix: persist user settings under sandbox`. Fork-owned body: `xai-grok-sandbox/src/user_config_writer.rs`. Thin wiring: sandbox `lib.rs` exports, shell `apply_sandbox` + `persist.rs`, user-guide `18-sandbox.md`, sandbox `Cargo.toml` `tempfile` dev-dep. Do not fold into slim. Do not lift `config.toml` off the kernel write-deny list to "fix" settings.
